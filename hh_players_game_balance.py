...
files_filter = lambda f: path.isfile(path.join(hh_data_dirname, f)) and f.endswith('.txt') and f.startswith('2020-09-03')
...
for index, summary in enumerate(summaries):
    ...
    for player_info in players_info:
        ...
        if balance != 0:
            ...
            p_balances_real = players_balances_real[nickname] if nickname in players_balances_real else []
            p_balances_real.append(balance)
            players_balances_real[nickname] = p_balances_real
...
for player in players_balances_real:
    p_balances_real = np.asarray(players_balances_real[player])
    print(f'nickname: {player}')
    print(f'sum: {p_balances_real.sum() : .2f}')
